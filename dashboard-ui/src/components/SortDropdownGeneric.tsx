...
interface SortDropdownGenericProps {
  table: string;
...
const SortDropdownGeneric = ({ table, sortOptions, title = "Sắp xếp" }: SortDropdownGenericProps) => {
  const router = useRouter();
  const searchParams = useSearchParams();
  const dropdownRef = useRef<HTMLDivElement>(null);
  const [isOpen, setIsOpen] = useState(false);
  const [sortBy, setSortBy] = useState(searchParams.get("sortBy") || sortOptions[0]?.key || "");
...
export default SortDropdownGeneric; 